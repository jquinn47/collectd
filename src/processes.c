...
static void ps_submit_type (derive_t value, const char* type)
{
	value_list_t vl = VALUE_LIST_INIT;
...
static int read_fork_rate_and_syscalls (void)
{
	...
	while (fgets (buffer, sizeof (buffer), proc_stat) != NULL)
	{
		...
		{
			if((status = parse_value (fields[1], &fr_value, DS_TYPE_DERIVE)) == 0)
				fr_value_valid = 1;
		}
		else if (fields_num >= 2 && (strcmp ("intr", fields[0]) == 0))
		{
			if((status = parse_value (fields[1], &intr_value, DS_TYPE_DERIVE)) == 0)
				intr_value_valid = 1;
		}
	...
	ps_submit_type (fr_value.derive, "fork_rate");

	// We derive the number of system calls from the total number
	// of processes that have been created (fork(), clone()), and
	// the total number of interrupts serviced since boot time

	ps_submit_type (fr_value.derive + intr_value.derive, "syscalls");
...
static int read_fork_rate (void)
{
	...
	ps_submit_type (result, "fork_rate");
	return (0);
}
#endif /* KERNEL_SOLARIS */
...
static int read_fork_rate_and_syscalls ()
{
	...
	ps_submit_type ((derive_t) cpu_totals.sysfork, "fork_rate");
	ps_submit_type ((derive_t) cpu_totals.syscall, "syscalls");
